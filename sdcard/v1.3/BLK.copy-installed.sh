...
# Rekomendasi : Jika perangkat anda mendukung penonaktifan folder Standar coba juga Skrip D2DF serupa dengan Skrip ini.
...
cat /sdcard/download/$FILE | xargs > $FILE1 2>/dev/null;

else
echo $(pwd)
cat ${FILE} | xargs > $FILE1 
fi
